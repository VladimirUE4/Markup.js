...
try {
  ...
  // Vérification préventive pour s'assurer que items existe et est un tableau
  if (context.items === undefined) {
    context.items = [];
  } else if (!(context.items instanceof Array)) {
    context.items = [context.items]; // Convertir en tableau si ce n'est pas déjà le cas
  }
} catch (err) {
...
let output;
try {
  // Configuration correcte pour Markup.js avec délimiteurs personnalisés
  const options = {
